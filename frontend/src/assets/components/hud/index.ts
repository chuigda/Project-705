import ascension_perkIcon from './ascension_perk.png'
import learn_skillIcon from './learn_skill.png'
import diplomacyIcon from './diplomacy.png'
import item_storageIcon from './item_storage.png'
import shopIcon from './shop.png'
import debugIcon from './debug.png'
import expand_menuIcon from './expand_menu.png'
import retract_menuIcon from './retract_menu.png'

export const menuIcons = {
   ascension_perk: ascension_perkIcon,
   learn_skill: learn_skillIcon,
   diplomacy: diplomacyIcon,
   item_storage: item_storageIcon,
   shop: shopIcon,
   debug: debugIcon,
   expand_menu: expand_menuIcon,
   retract_menu: retract_menuIcon
}

export default menuIcons
