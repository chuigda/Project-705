...
import scopeFunctions from '@app/core/game_context/scope'
import connectFunctions, { Signal } from '@app/core/connect'
import eventFunctions from '@app/core/events'
import grantFunctions from '@app/core/skill'
import propertyFunctions from '@app/core/property'
import variableFunctions from '@app/core/variable'
import uiFunctions, { BubbleMessage, SimpleDialog } from '@app/core/ui'
import ascensionPerkFunctions from '@app/core/ascension_perk'
import modifierFunctions from '@app/core/modifier'
import storeItemFunctions from '@app/core/store_item'
import activityFunctions from '@app/core/activity'

import { PlayerProperty, PlayerStatus, PropertyId } from '@app/core/game_context/player'
import { ShopStatus } from '@app/core/game_context/shop'
import { EventHooks } from '@app/core/game_context/event_hook'

export * from '@app/core/game_context/event_hook'
export * from '@app/core/game_context/player'
export * from '@app/core/game_context/shop'
...
export class GameState {
   ...
   /// 经过计算的可用/不可用飞升项目列表
   computedAscensionPerks: ComputedAscensionPerks = {
      available: {},
      unavailable: {}
   }
...
export class GameContext {
   ...
   /// 触发一个事件
   public triggerEvent(event: MaybeInlineEvent, ...args: any[]) {
      eventFunctions.triggerEvent(this, event, ...args)
   }

   /// 添加一个静态修正
   public addModifier(modifier: Ident) {
      modifierFunctions.addModifier(this, modifier)
   }

   /// 移除一个静态修正
   public removeModifier(modifier: Ident) {
   ...
   public grantSkill(skill: Ident, force?: boolean) {
      grantFunctions.grantSkill(this, skill, force)
   }

   /// 获得一项活动
   public addActivity(activity: Ident) {
      activityFunctions.addActivity(this, activity)
   }

   /// 增加飞升槽
   public addAscensionPerkSlot(count: number) {
   ...
   public activateAscensionPerk(ascensionPerk: Ident, force?: boolean) {
